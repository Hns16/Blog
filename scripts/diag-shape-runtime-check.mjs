...
const REQUESTED_PORT = Number(process.env.DRILL_PORT || 3322);
...
async function selectPort(preferred) {
  ...
  for (let i = 0; i < 20; i += 1) {
    const candidate = 3900 + Math.floor(Math.random() * 500);
...
async function main() {
  ...
  try {
    ...
    const res = await fetch(`${BASE}/api/admin/diag/snapshot?limit=5`, {
      method: "GET",
      headers: {
        origin: BASE,
        referer: `${BASE}/admin`,
    ...
    if (res.status !== 200) throw new Error(`diag_status_${res.status}`);
    const json = await res.json();
    if (!json?.health) throw new Error("missing_health");
    if (!json?.audit) throw new Error("missing_audit");
    if (!Array.isArray(json?.events)) throw new Error("missing_events");

    console.log(
      JSON.stringify(
        {
          ok: true,
          checks: ["health", "audit", "events"],
